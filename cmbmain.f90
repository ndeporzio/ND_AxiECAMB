!     This is the main CAMB program module.
!
...
module CAMBmain
  ...
  real(dl), dimension(:,:,:), allocatable :: Src, ddSrc, deltaBCSrc, dd_deltaBCSrc !Sources and second derivs !RL 090323 added delta functions at the boundary to address discontinuity in sources - and note that these second derivatives are for interpolations across k space, not interpolations across time
  ...
  real(dl),parameter :: qmin0=0.1_dl !RL changed to 1000.0 for glitch testing, default is 0.1
  ...
  real(dl) :: fixq = 0._dl!2.189281413133309E-005_dl!0.02563682641469507_dl!!2.3470372925e-6_dl! ! !Debug output of one q (RL: which is the scale k; if the value is 0, there's no fixing)
    ...
    if (CP%WantCls) then
       ...
       do q_ix= 1,Evolve_q%npoints
          if (global_error_flag==0) then 
             ...
             !write(09192300, '(36E52.42)') EV%q, deltaBCSrc(q_ix, 1, 1), deltaBCSrc(q_ix, 1, 2)
          end if
       end do
       !        clock_stop = 0
       ...
       !$OMP END PARAllEl DO
       !!call cpu_time(clock_stop) ! RH timing 
       !!        print*, 'timing for DoSourceK', clock_stop - clock_start
       !!close(09192300)
...
!!! RH axions need to be called again here !!!
    ...
    !!print*, 'timing for the whole of cmbmain', clock_totstop - clock_totstart ! RH timing
    !RL 092023 close all the txt files opened
    !!do i_output = 1, size(l_outputs)
    !!   l_output = l_outputs(i_output)
    !!   close(unit=09202300+l_output)  
    !!end do
    !!close(unit=09192300)
    !!close(unit=09192301)
    !RL 092023 --------------
...
!!!write(*, *) 'TRY: CP%Transfer%kmax, qmin, MT%num_q_trans, q_transfer(MT%num_q_trans)?', CP%Transfer%kmax, qmin, MT%num_q_trans, q_transfer(MT%num_q_trans)
    ...
    if (CP%WantCls) then
       ...
       !write(*, *) 'Rayne, what is MT%num_q_trans after this block?', MT%num_q_trans
       ...
       !write(*, *) 'And then what is q_transfer?', q_transfer !RL
    else
       Evolve_q%npoints = 0
       call Transfer_Allocate(MT)
  ...
  function GetTauStart(q)
    ...
    if(CP%Omegaax>0) then
       ...
       !write(*, *) 'a_osc is', a_osc
       !write(*, *) 'the corresponding tauosc obtained from GetTauStart is', tauosc !RL wants to know when tauosc is calculated (and then compare that with the corresponding a_osc that I got previously - it's very very different and this one should be incorrect. I don't know what's going on here, but apparently the tauosc is not calculated )
       !DM:                                                                                                                      
       arad=((a_osc**3.0d0)*((grhog+grhonu)*0.01d0)/(grhoax))**(1.0d0/4.0d0)
       abar=((a_osc**3.0d0)*((grhob)*0.01d0)/(grhoax))**(1.0d0/3.0d0)
       !write(*, *) 'Rayne, CP%aeq, adotrad, dsqrt(1.0d0+CP%aeq*0.01d0/adotrad), (-1.0d0+dsqrt(1.0d0+CP%aeq*0.01d0/adotrad))/om/2.0d0', CP%aeq, adotrad, dsqrt(1.0d0+CP%aeq*0.01d0/adotrad), (-1.0d0+dsqrt(1.0d0+CP%aeq*0.01d0/adotrad))/om/2.0d0
  ...
  subroutine SetkValuesForSources
    ...
    SourceAccuracyBoost = AccuracyBoost!*10._dl!*CP%dfac/10._dl !RL 102123
...
!!!write(*, *) 'RAd 4'
    ...
    !write(112301, '(36e52.42,\)') Evolve_q%points !RL
  ...
  subroutine CalcScalarSources(EV,taustart)
    ...
    w=0
    y=0
    !write(*, *) 'What is taustart initially?', taustart !RL
    ...
    tau=taustart
    ind=1 
    !write(*, *) 'And what is tau initially?', tau !RL
    ...
    do j=2,TimeSteps%npoints
       ...
       if (.not. DebugEvolution .and. (EV%q*tauend > max_etak_scalar .and. tauend > taurend) &
          ...
          call GaugeInterface_EvolveScal(EV,tau,y,tauend,tol1,ind,c,w)
          if (global_error_flag/=0) return
          !!write(*, *) 'output called, EV%q_ix', EV%q_ix
...
!!!write(*, *) 'RL, is TransferOut called? and Evolve_q%npoints+1, MT%num_q_trans, MT%q_trans(Evolve_q%npoints+1), MT%q_trans(MT%num_q_trans)?', Evolve_q%npoints+1, MT%num_q_trans, MT%q_trans(Evolve_q%npoints+1), MT%q_trans(MT%num_q_trans)
  ...
  subroutine GetTransfer(EV,tau)
    ...
    do i=1,CP%Transfer%num_redshifts
       !write(*, *) 'Rayne, within CP%Transfer%num_redshifts in GetTransfer, tau, tautf(i)', tau, tautf(i)
  ...
  subroutine InitSourceInterpolation
    ...
    do  i=1,TimeSteps%npoints
       do j=1, SourceNum
          ...
          !!write(*, *) 'in InitSourceInterpolation:'
          !!write(*, *) 'Evolve_q%points:',Evolve_q%points
          !!write(*, *) 'Src(1,j,i):',Src(1,j,i)
          !!write(*, *) 'Evolve_q%npoints:', Evolve_q%npoints
          !!write(*, *) 'ddSrc(1,j,i)', ddSrc(1,j,i)
       end do
    end do
    !$OMP END PARALLEL DO
  ...
  subroutine SetkValuesForInt
    ...
    IntSampleBoost=AccuracyBoost!*10._dl !RL 020624
...
!!    end if
    ...
    if (.not.CP%flat) then
       !write(*, *) 'RL, nonflat, CP%omegak, CP%curv', CP%omegak, CP%curv
!!!---       write(*, *) 'RL, nonflat, IV%ddSource_q(1:5,1)', IV%ddSource_q(1:5,1)
       do i=1, SourceNum          
          call spline(TimeSteps%points,IV%Source_q(1,i),TimeSteps%npoints,&
               spl_large,spl_large,IV%ddSource_q(1,i))
       end do
!!!---       write(*, *) 'RL, nonflat, after spline, IV%ddSource_q(1:5,1)', IV%ddSource_q(1:5,1)
    end if
  ...
  subroutine DoSourceIntegration(IV) !for particular wave number q
    ...
    if (CP%flat) then
       ...
       do j=1,lSamp%l0
          ll=lSamp%l(j)
          !-!-if (ll .gt. 2 .and. ll .lt. 6 .and. IV%q_ix .gt. 20 .and. IV%q_ix .lt. 24) then
          !-!-write(*, *) 'IV%q_ix, ll, llmax, ll>llmax', IV%q_ix, ll, llmax, ll>llmax
          !-!-end if
          if (ll>llmax) exit
  ...
  subroutine DoFlatIntegration(IV, llmax)
    ...
    !     timestep
    !write(*, *) 'Rayne, in DoFlatIntegration, IV%q, IV%metricdeltas_q(1, 1), IV%metricdeltas_q(2, 1)'
    !write(*, '(36e52.42)'), IV%q, IV%metricdeltas_q(1, 1), IV%metricdeltas_q(2, 1)

    !write(09042301, '(36e52.42)') IV%q, IV%metricdeltas_q(1, 1), IV%metricdeltas_q(2, 1)
    !write(*, *) 'Rayne, TimeSteps%points(IV%SourceSteps), CP%tau0', TimeSteps%points(IV%SourceSteps), CP%tau0
!!!!do n= 1, TimeSteps%npoints
!!!!   if (n .eq. 1) then
!!!!      write(*, *) 'n = 1, TimeSteps%dpoints(n), TimeSteps%points(n+1) - TimeSteps%points(n), their fractional difference', TimeSteps%dpoints(n), TimeSteps%points(n+1) - TimeSteps%points(n), TimeSteps%dpoints(n)/(TimeSteps%points(n+1) - TimeSteps%points(n)) - 1._dl
!!!!   else if (n .eq. TimeSteps%npoints) then
!!!!      write(*, *) 'n = -1, TimeSteps%dpoints(n), TimeSteps%points(n) - TimeSteps%points(n-1), their fractional difference', TimeSteps%dpoints(n), TimeSteps%points(n) - TimeSteps%points(n-1), TimeSteps%dpoints(n)/(TimeSteps%points(n) - TimeSteps%points(n-1)) - 1._dl
!!!!   else
!!!!      write(*, *) 'other n cases, TimeSteps%dpoints(n), (TimeSteps%points(n+1) - TimeSteps%points(n-1))/2._dl, their fractional difference', TimeSteps%dpoints(n), (TimeSteps%points(n+1) - TimeSteps%points(n-1))/2._dl, TimeSteps%dpoints(n)/((TimeSteps%points(n+1) - TimeSteps%points(n-1))/2._dl) - 1._dl
!!!!   end if

!!!!end do
    !write(*, *) 'Rayne, is BessRanges enough for tau0?', Ranges_indexOf(BessRanges,0._dl)
       ...
       if (SourceNum==2) then
          ...
          if (DoInt) then
             do n= Ranges_IndexOf(TimeSteps,tmin),min(IV%SourceSteps,Ranges_IndexOf(TimeSteps,tmax))
                ...
                J_l=a2*ajl(bes_ix,j)+(1-a2)*(ajl(bes_ix+1,j) - ((a2+1) &
                     *ajlpr(bes_ix,j)+(2-a2)*ajlpr(bes_ix+1,j))* fac(n)) !cubic spline
                !!write(*, *) 'Rayne, BessRanges%points(bes_ix), lSamp%l(j), ajl(bes_ix, j)', BessRanges%points(bes_ix), lSamp%l(j), ajl(bes_ix, j)


                !if (any(lSamp%l(j) == l_outputs)) then !RL 092123
                !   i_output = findloc(l_outputs, lSamp%l(j), dim=1)           
                !   xf_n = abs(IV%q*(CP%tau0-TimeSteps%points(n)))
                !write(*, *) 'Rayne, IV%q, CP%tau0,TimeSteps%points(n), IV%q*(CP%tau0-TimeSteps%points(n)), xf_n', IV%q, CP%tau0,TimeSteps%points(n), IV%q*(CP%tau0-TimeSteps%points(n)), xf_n
                !   l_float = lSamp%l(j)
                !   if (abs(l_float - xf_n) .le. 5._dl) then
                !      write(*, *) 'Rayne, lSamp%l(j), l_float, abs(IV%q*(CP%tau0-TimeSteps%points(n))), J_l', lSamp%l(j), l_float, xf_n, J_l
                !      write(unit_nums(i_output), '(I6, 36E52.42, 36E52.42)') lSamp%l(j), l_float, xf_n, J_l
                !   end if
                !end if
                J_l = J_l*TimeSteps%dpoints(n)
...
!!!write(010423, '(36E52.42E3)') TimeSteps%points(n), TimeSteps%dpoints(n), J_l, IV%Source_q(n, 1), sums(1), vis(n), dvis(n), ddvis(n)
                !!end if
                !!if (n .eq. min(IV%SourceSteps,Ranges_IndexOf(TimeSteps,tmax)).and. lSamp%l(j) .eq. 1000) then
                !!  write(*, *) 'Rayne, output IV%q_ix, IV%q, lSamp%l(j), J_l, TimeSteps%points(n)', IV%q_ix, IV%q, lSamp%l(j), J_l, TimeSteps%points(n)
                !! write(*, *) 'writing'
                !!  write(02122402, '(36E52.42E3)') IV%q, TimeSteps%points(n), IV%Source_q(n, 1), sums(1)
                !!sums1_ab1[:-1]-(Jldpoint_ab1/Tdpoint_ab1)[:-1]*IVsourceq1_ab1[:-1]*np.diff(Tpoint_ab1)/2. The last sum shouldn't have a problem since it's the last one
                !!end if
             end do
          end if
       ...
       if (associated(IV%metricdeltas_q) .and. allocated(deltaBCSrc)) then
          ...
          !!fac_osc_m=BessRanges%points(besix_osc_m+1)-BessRanges%points(besix_osc_m)
          !!aa_osc_m=(BessRanges%points(besix_osc_m+1)-xf_osc_m)/fac_osc_m
          !!fac_osc_m=fac_osc_m**2._dl*aa_osc_m/6._dl
          !!Jl_osc_m=aa_osc_m*ajl(besix_osc_m,j)+(1-aa_osc_m)*(ajl(besix_osc_m+1,j) - ((aa_osc_m+1) &
          !!     *ajlpr(besix_osc_m,j)+(2-aa_osc_m)*ajlpr(besix_osc_m+1,j))*fac_osc_m)

          !!xf_osc_p=abs(IV%q*(CP%tau0-CP%tau_osc*(1._dl + Jl_eps)))
          !!besix_osc_p=Ranges_indexOf(BessRanges,xf_osc_p)
          !!fac_osc_p=BessRanges%points(besix_osc_p+1)-BessRanges%points(besix_osc_p)
          !!aa_osc_p=(BessRanges%points(besix_osc_p+1)-xf_osc_p)/fac_osc_p
          !!fac_osc_p=fac_osc_p**2._dl*aa_osc_p/6._dl
          !!Jl_osc_p=aa_osc_p*ajl(besix_osc_p,j)+(1-aa_osc_p)*(ajl(besix_osc_p+1,j) - ((aa_osc_p+1) &
          !!     *ajlpr(besix_osc_p,j)+(2-aa_osc_p)*ajlpr(besix_osc_p+1,j))*fac_osc_p)

          !!write(*, *) 'Rayne, analytical Jldot, numerical Jldot, their fractional difference, whether the difference is smaller than 1e-6', dotJl_osc, (Jl_osc_p - Jl_osc_m)/(CP%tau_osc*Jl_eps*2._dl), dotJl_osc/((Jl_osc_p - Jl_osc_m)/(CP%tau_osc*Jl_eps*2._dl)) - 1._dl, (abs( dotJl_osc/((Jl_osc_p - Jl_osc_m)/(CP%tau_osc*Jl_eps*2._dl)) - 1._dl) .lt. 1.d-6)!obtain the approximate 1st conformal time derivative
          !!dotJl_osc = (Jl_osc_p - Jl_osc_m)/(CP%tau_osc*Jl_eps*2._dl)
          !if (lSamp%l(j) == 3200) then
          !   write(*,'(36e52.42)')  xf_osc, lSamp%l(j), Jl_osc, -dotJl_osc/IV%q
          !end if

          !!write(*, *) 'Rayne, xf_osc, lSamp%l(j), Jl_osc, pJl_osc', xf_osc, lSamp%l(j), Jl_osc, -dotJl_osc/IV%q
          !Add the boundary condition to the integral - only scalar at the point. Note that the boundary condition doesn't need to be multiplied by the measure [i.e. TimeSteps%dpoints(n)]
          !write(*, *) 'Rayne, CP%expmmu_tauosc*Jl_osc*IV%metricdeltas_q(1, 1) + CP%expmmu_tauosc*dotJl_osc*IV%metricdeltas_q(2, 1)', CP%expmmu_tauosc*Jl_osc*IV%metricdeltas_q(1, 1), CP%expmmu_tauosc*dotJl_osc*IV%metricdeltas_q(2, 1)
          !RL 092023 writing j_l to file
          !if (any(lSamp%l(j) == l_outputs)) then
          ! Find index corresponding to ell in l_outputs
          !   i_output = findloc(l_outputs, lSamp%l(j), dim=1)

          !print *, "j is, unit number is: ", j, unit_nums(i_output)
          !write(*, *) j, IV%q, Jl_osc, dotJl_osc
          !write(*, *) 'Rayne, calling j output, IV%q', IV%q
    ...
    if ((CP%WantScalars)) then !Do Scalars
       ...
       if (DoInt) then
          if ((nstart < min(TimeSteps%npoints-1,IV%SourceSteps)).and.(y1dis > miny1)) then
             ...
             y2=y2dis
             nnow=nstart
!!!---               write(*, *) 'WantScalarCls, dissipative region, nstart, min(TimeSteps%npoints-1,IV%SourceSteps)), l, TimeSteps%Count', nstart, min(TimeSteps%npoints-1,IV%SourceSteps), l, TimeSteps%Count
...
!!!---                     write(*, *) 'nnow, nbot, nnow>nbot, DoRangeInt is called', nnow, nbot
       ...
       !!write(*, *) 'scalars,  ThisCT%Delta_p_l_k(1:SourceNum,j,IV%q_ix)',  ThisCT%Delta_p_l_k(1:SourceNum,j,IV%q_ix)
    end if !Do Scalars
  ...
  subroutine DoRangeInt(IV,chi,chiDisp,nstart,nend,dtau,nu,l,y1,y2,out)
    ...
    !write(*, *) 'Is the sources redefined here???', sources !RL being desperate
    ...
    !write(*, *) 'Under curved universe, CP%tau_osc, CP%tau0, nstart, TimeSteps%points(nstart), nend, TimeSteps%points(nend)', CP%tau_osc, CP%tau0, nstart, TimeSteps%points(nstart), nend, TimeSteps%points(nend)
